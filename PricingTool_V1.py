...
def load_data():
    ...
    name = "Packing"
    excel_file = fetch_excel_from_sharepoint(name)
    df_Pack = pd.read_excel(excel_file,sheet_name=None, engine='openpyxl')
    
    
    return df_rcp, df_wages, df_SetUpTimes, df_RawIngredients, df_RCP_Boms,df_Base,df_BaseRR,df_BaseW,df_Top, df_Pack

def main():
    # Load the data
    df_rcp, df_wages, df_SetUpTimes, df_RawIngredients, df_RCP_Boms,df_Base,df_BaseRR,df_BaseW, df_Top, df_Pack = load_data()
    df_Pack1 = df_Pack['RunRates']
    df_Pack2 = df_Pack['PackParts']
    df_Pack3 = df_Pack['PartsCost']
    df_Pack4 = df_Pack['AuxillaryMachines']
    ...
    page = st.sidebar.radio('Navigate:', [ 'Base Selection', 'Packaging'])
    ...
    if page == 'Base Selection':
        ...
        st.divider()
        st.markdown(":red-background['Results: Prices per stage']") 
    ...
    if page=="Packaging":
        #cookie_data=st.session_state["cookie_data"]  
        #res1=st.session_state["res1"]
        
        st.subheader("For One ITM type/code")
        num_itms = st.number_input("Number of ITMS ordered", min_value=0, step=1)
        num_var = st.number_input("Enter number of cookie variations in one unit of ITM:", min_value=0, step=1)
        columns1 = ["Name", "#"]
        tab = pd.DataFrame([["",0]] * num_var, columns=columns1)
        st.write("Hom many cookies in each variation of this ITM:")
        tab1 = st.data_editor(tab, num_rows="fixed")
        tab1['QtyToProduce']=tab1['#']*num_itms
        st.write(tab1)
        
        sup_pack = 1
        comb4=pd.DataFrame() 
        w_op_pack = df_wages[df_wages['Description']=='Machine Operator']['Value'].values[0]
        w_pack = df_wages[df_wages['Description']=='Packaging']['Value'].values[0]
        w_sup = df_wages[df_wages['Description']=='Line Supervisor']['Value'].values[0]
        set_up_pk= df_SetUpTimes[df_SetUpTimes['Department']=='Packing']
        fam1=df_Pack1['Family'].drop_duplicates()
        # st.write(cookie_data)
        # st.write(res1)
        # if f"fam_{i}" not in st.session_state:
        #     st.session_state[f"bot_{i}"] = "0"
        fam = st.radio('Select Packing Family:', fam1, horizontal=True) #, key=f"fam_{i}")
        p_rr= df_Pack1[df_Pack1['Family']==fam]
        p_code = st.radio('Select Pack Code:', p_rr['Pack Line Code'], horizontal=True)
        
        p_rr1= df_Pack1[df_Pack1['Pack Line Code']==p_code]# Create a new column with matching column names per row
        p_rr1['Lines'] = p_rr1.apply(
            lambda row: [col for col in p_rr1.columns if row[col] == "Y"],
            axis=1
        )
        p_rr= p_rr1.drop(['Line 1','Line 2','Line 3','Line 4','Line 5'], axis=1)
        
        pack1= df_Pack2[df_Pack2['Pack Line Code']==p_code]
        pack1['Quantity']=0   
        
        #st.subheader("Fill or edit your data below:")
        st.write("Fill #pieces required for following packaging parts")
        edited_df_p = st.data_editor(pack1, num_rows="fixed")
        
        pack2= df_Pack3[['Part#','Cost']]
        pack11=pd.merge(edited_df_p, pack2, left_on="Packaging Item Code", right_on="Part#", how="left")
        pack11['MaterialCost/Case']=pack11['Cost']*pack11['Quantity']
        st.write(pack11)
        pack11['Qty/Case']=p_rr['QTY/Case'].values[0] 
        n_cases=num_itms /p_rr['QTY/Case'].values[0]
        pack_m = sum(pack11['MaterialCost/Case'])/ pack11['Qty/Case'].values[0] #material cost for each item in a case
        st.write("Material Cost of each Case",sum(pack11['MaterialCost/Case']))
        st.write("Material Cost of each ITM",pack_m)
        
        op=df_Pack4[df_Pack4['Pack Line Code']==p_code]['FW'].values[0]
        p_rr['SetUp']=set_up_pk['Shift SetUp Time(mins)'].values[0]
        p_rr['TotalCases']=n_cases
        p_rr['JobTime']= 1*n_cases*60/p_rr['Run Rate']
        p_rr['Labour&SetupCost/ReqdCase']=((p_rr['# of People']-op)*w_pack + sup_pack*w_sup + op*w_op_pack )* (p_rr['SetUp'] + p_rr['JobTime']) / 60   #each line has a line supervisor
        st.write("Labour&Setup Cost of each Case", p_rr['Labour&SetupCost/ReqdCase'].values[0]/n_cases)
        cost1 = p_rr['Labour&SetupCost/ReqdCase'].values[0]/n_cases #labour cost per case
        p_rr['Labour&SetupCost/ITM']=cost1/p_rr['QTY/Case']
        st.write("Labour&Setup Cost of each ITM", cost1/p_rr['QTY/Case'].values[0])
        p_rr['MaterialCost/ReqdCase']=sum(pack11['MaterialCost/Case'])*n_cases
        p_rr['MaterialCost/ITM']=pack_m
        st.write(p_rr)
        skill=df_Pack4[df_Pack4['Pack Line Code']==p_code].iloc[:,3:14]
        skill['Others']=p_rr['# of People'].values[0]-skill.sum(axis=1)
        skill1=skill.transpose()
        skill1.columns = ['#people']
        skill1.index.name = 'Skill'
        skill1=skill1[skill1['#people']>0]      
        st.write("Skilled People",skill1)
